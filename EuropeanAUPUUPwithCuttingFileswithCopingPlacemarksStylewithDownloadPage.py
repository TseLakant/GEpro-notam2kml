...
def download_page():
    with sync_playwright() as p:
        ...
        print(f"HTML код успешно сохранен в файл {HTML_FILE}")
        browser.close()
...
def process_ge_pro_kml(input_path, output_path, folders_to_copy, regions_dict):
    ...
    for folder in folders:
        ...
        if folder_name in folders_to_copy:
            print(f"📦 Копируем папку полностью: {folder_name}")
            # Ничего не делаем, она остается в дереве со всем содержимым
        else:
            print(f"🔧 Обработка содержимого папки: {folder_name}")
            ...
            print(f"Найдено Placemark'ов для обработки: {len(placemarks)} в файле {folder_name}")
